// src/pages/FinanciamientoPropiedad.jsx
...
function getPropertyTitle(property = {}, journey = {}) {
  return (
    ...
    journey?.propiedadSeleccionada?.titulo ||
    journey?.propiedadSeleccionada?.nombre ||
    "Propiedad elegida"
  );
}
...
function getPropertyPrice(property = {}, journey = {}) {
  return (
    ...
    moneyNumber(property?._normalizedPrice) ??
    moneyNumber(journey?.propiedadPrecio) ??
    moneyNumber(journey?.selectedPropertyPrice) ??
    moneyNumber(journey?.propiedadSeleccionada?.precio) ??
    moneyNumber(journey?.propiedadSeleccionada?.price) ??
    null
  );
}
...
function getAvailableEntry({ snapshot, journey, property }) {
  return (
    ...
    moneyNumber(journey?.form?.entradaDisponible) ??
    moneyNumber(journey?.form?.entrada) ??
    moneyNumber(journey?.form?.ahorroDisponible) ??
...
function getRequiredEntry(propertyPrice, property = {}) {
  return (
    ...
    moneyNumber(property?.entradaMinima) ??
...
function normalizeProductId(value) {
  ...
  if (text.includes("BIESS") || text.includes("CREDICASA")) {
    return "BIESS";
  }
...
function collectMortgageRoutes({
  ...
  pushArray(snapshot?.mortgageMarketplace?.alsoAppliesForCurrentGoal);
  pushArray(snapshot?.mortgageMarketplace?.couldWorkIfAdjusted);
  pushOne(snapshot?.mortgageMarketplace?.bestForCurrentGoal);
  ...
  return (
    <div
      style={{
        padding: 16,
        borderRadius: 20,
        background: accent ? "rgba(37,211,166,0.11)" : "rgba(15,23,42,0.50)",
        border: accent
...
export default function FinanciamientoPropiedad() {
  ...
  const loanAmount =
    propertyPrice != null && entryAvailable != null
      ...
      : propertyPrice != null && entryRequired != null
      ? Math.max(0, propertyPrice - entryRequired)
      : null;
  ...
  const [saved, setSaved] = useState(() => {
    if (
      storedSelectedMortgageRoute?.status !== "selected" &&
      storedSelectedMortgageRoute?.status !== "confirmed"
    ) {
      return false;
    }
...
entryAvailable,
entryRequired,
loanAmount: selectedRoute.loanAmount ?? loanAmount,
routeObjective,
routeGap,
      ...
      annualRate: selectedRoute.tasaAnual,
      plazoMeses: selectedRoute.plazoMeses,
      termMonths: selectedRoute.plazoMeses,
      cuota: selectedRoute.cuota,
      montoPrestamo: selectedRoute.loanAmount,
      score: selectedRoute.score ?? null,

      productLabel: selectedRoute.producto,
      providerLabel: selectedRoute.provider || selectedRoute.producto,
      mortgageId: selectedRoute.productId || selectedRoute.producto,
    ...
    const nextJourney = {
      ...
      mortgageRoute: payload,
      mortgageRouteConfirmedAt: now,
    ...
    <HabitaShell maxWidth={980}>
      <div style={{ paddingBottom: 32 }}>
            ...
            >
              {routeGap > 0 ? (
                <>
                  ...
                  {isCloseToRoute ? (
                    <>
                      Esta propiedad está cerca, pero todavía queda una
                      diferencia referencial de{" "}
                      <strong>{moneyUSD(routeGap)}</strong>.
                    </>
                  ) : (
                    <>
                      Esta propiedad está por encima de tu ruta objetivo. La
                      diferencia referencial es de{" "}
                      <strong>{moneyUSD(routeGap)}</strong>. Para acercarte,
                      podrías necesitar mayor entrada, reducir deudas o comparar
        ...
        <Card style={{ marginTop: 18, padding: 22 }}>
          ...
          {!loanAmount ? (
            ...
            <>
              ...
              >
                {routes.some(
                  (route) => route.producto === "VIS" || route.producto === "VIP"
                ) ? (
                  <>
                    VIS y VIP son rutas preferenciales que normalmente se
                    canalizan a través de entidades financieras participantes.
                    La categoría aplicable depende del valor de la vivienda, si
                    es primera vivienda y de las condiciones vigentes.
                  </>
                ) : (
                  <>
                    Según la información declarada, esta propiedad se muestra
                    con una ruta hipotecaria tradicional. Las condiciones finales
                    dependen de cada entidad financiera.
                  </>
                )}
              </div>
        ...
        <div style={{ marginTop: 18, display: "grid", gap: 10 }}>
          <PrimaryButton
            onClick={() => nav(propertyId ? `/property/${propertyId}` : "/match")}
          >
