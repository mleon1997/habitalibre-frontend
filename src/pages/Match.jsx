...
function MarketplaceResultsPane({
  ...
  const title = selectedPropertyId
    ? "Viviendas alrededor de tu propiedad base"
    : "Viviendas recomendadas para ti";

  const subtitle = selectedPropertyId
    ? "Opciones comparables tomando como referencia la propiedad que elegiste."
    : "Opciones ordenadas según tu capacidad, entrada y ruta estimada.";

  const modeLabel =
    propertyMode === "strict" ? "Mejores opciones" : "Alternativas ampliadas";
...
function handleOpenMortgageDetail(route = mortgageDetailRoute) {
  const selectedRoute = buildMortgageRoutePayload(
    route,
    "match_hipoteca_detail"
  );

  if (!selectedRoute) return;

  saveOwnedData(LS_SELECTED_MORTGAGE_ROUTE, {
    ...selectedRoute,
    raw: route,
    product: route?.product || route?.raw?.product || null,
    termOptionsYears:
      route?.termOptionsYears || route?.raw?.termOptionsYears || null,
    termOptionsMonths:
      route?.termOptionsMonths || route?.raw?.termOptionsMonths || null,
    termPolicyLabel:
      route?.termPolicyLabel ||
      route?.raw?.termPolicyLabel ||
      route?.product?.term?.policyLabel ||
      route?.raw?.product?.term?.policyLabel ||
      null,
    termNotes:
      route?.termNotes ||
      route?.raw?.termNotes ||
      route?.product?.term?.notes ||
      route?.raw?.product?.term?.notes ||
      null,
  });

  navigate("/hipoteca-detalle");
}
    ...
    style={{
      padding: 16,
                ...
                >
                  ...
                  >
                    {goalValue != null
                      ? `Tu meta original sigue siendo ${moneyUSD(
                          goalValue
                        )}, pero hoy vemos mejores opciones concretas cercanas.`
