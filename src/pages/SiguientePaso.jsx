...
function getReadinessMeta({ caseReadinessStatus, hasChosenProperty }) {
  if (caseReadinessStatus === "no_listo") {
    return {
      ...
      path: hasChosenProperty ? "/checklist-documentos" : "/match",
  ...
  async function submitCaseActivation() {
    ...
    if (!token) {
      throw buildAuthError(
        "Tu sesión expiró. Vuelve a iniciar sesión para enviar tu caso."
      );
    }
    ...
    try {
      res = await fetch(`${API_BASE}/casos-activacion`, {
        method: "POST",
        headers: {
      ...
      throw new Error(
        "No pudimos conectar con HabitaLibre en este momento. Intenta nuevamente."
      );
    }
    ...
    if (!res.ok) {
      const message =
        data?.error ||
        data?.message ||
        "No se pudo enviar tu caso a revisión HabitaLibre.";
  ...
  async function handlePrimaryAction() {
    ...
    try {
      ...
      if (error?.code === "AUTH_EXPIRED") {
        setSubmitError(
          "Tu sesión expiró. Te llevaremos a iniciar sesión para poder enviar tu caso."
        );
